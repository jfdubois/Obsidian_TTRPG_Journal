module.exports = async (params) => {
    try {
        ...
        // 2. Select target (who receives healing)
        ...
        // 3. Healing amount
        const healStr = await inputPrompt("Healing amount:", "0");
        const healAmount = parseInt(healStr) || 0;
        if (healAmount <= 0) {
            new Notice("⚠️ Invalid healing amount!");
            return;
        }
        ...
        // If target is a player (character), just log it - don't track HP
        if (target.value.type === "character") {
            const logEntry = helpers.formatLogEntry(round, sourceName, "heal", targetName, healAmount);
            await addCombatLog(app, file, logEntry);
            new Notice(`💚 ${sourceName} healed ${targetName} for ${healAmount} HP!`);
            return;
        }

        // For monsters, track HP
        ...
        const newHp = Math.min(target.value.maxHp, target.value.currentHp + healAmount);
        const actualHealing = newHp - target.value.currentHp;
        ...
        // Add to combat log
        const logEntry = helpers.formatLogEntry(round, sourceName, "heal", targetName, actualHealing);
        await addCombatLog(app, file, logEntry);
        ...
        new Notice(`💚 ${sourceName} healed ${targetName} for ${actualHealing} HP!`);

    } catch (error) {
        console.error("💥 combatHeal error:", error);
